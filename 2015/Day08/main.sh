...
function part_two () {
    ...
    for line in $@; do
        code=$(( ${#line} ))
        new_string=4 # will always need an additional 4 characters for the wrapping quotes
        # echo "${line}"
        for (( i=1; i<$(( code - 1)); i+=1 )); do
            char="${line:$i:1}"
            if [[ $char == "\\" ]]; then
