...
DATA = """toggle 461,550 through 564,900
...
def toggle(grid, corner1, corner2, part2=False):
	corner1 = corner1.split(",")
	corner2 = corner2.split(",")
	for y in range(int(corner1[1]), int(corner2[1])+1):
		for x in range(int(corner1[0]), int(corner2[0])+1):
			if(part2): grid[y][x] += 2
			else: grid[y][x] = int(not grid[y][x])
	return grid

def power(grid, corner1, corner2, status, part2=False):
	corner1 = corner1.split(",")
	corner2 = corner2.split(",")
	for y in range(int(corner1[1]), int(corner2[1])+1):
		for x in range(int(corner1[0]), int(corner2[0])+1):
			if(part2): 
				...
				if(grid[y][x] < 0):
					grid[y][x] = 0
			else: grid[y][x] = status
	return grid

def num_powered(grid):
	powered = 0
...
def part_one(grid, data, part2=False):
	for cmd in data:
		cmd = cmd.split(" ")
		match cmd[0]:
			case 'toggle':
				grid = toggle(grid, cmd[1], cmd[3], part2)
			case 'turn':
				status = None
				if(cmd[1] == 'off'):
					if(part2): status = -1
					...
					status = 1
				grid = power(grid, cmd[2], cmd[4], status, part2)
	return num_powered(grid)

def part_two(grid, data):
	return part_one(grid, data, True)

if __name__ == "__main__":
	print(part_one(gen_grid(), DATA))
	print(part_two(gen_grid(), DATA))
