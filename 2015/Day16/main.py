...
DATA = """Sue 1: goldfish: 6, trees: 9, akitas: 0
...
Sue 500: perfumes: 4, cars: 9, trees: 4""".split("\n")
...
def parse_clues(data):
	...
	for clue in data:
		clue = clue.split(": ")
		parsed[clue[0]] = int(clue[1])
	return parsed

def parse_sues(data, clues):
	sues = []
	for sue in data:
		new_sue = {}
		sue = re.findall('\\D+: \\d+', sue)
		for clue in sue:
			clue = clue[2:].split(": ")
			new_sue[clue[0]] = int(clue[1])
		sues.append(new_sue)
	return sues

def filter1(data, clues):
	candidates = {}
	for i, sue in enumerate(data):
		i+=1
		for clue in clues:
			if clue in sue:
				# if clues aligns with Sue's info she's a canidate
				if(sue[clue] == clues[clue]):
					candidates[str(i)] = sue
				# if she was a candidate, she no longer is
				else:
					if(str(i) in candidates):
						candidates.pop(str(i))
					break
	return candidates

def filter2(data, clues):
	candidates = {}
	for i, sue in enumerate(data):
		i+=1
		bad = False
...
def part_one(data, clues):
	return filter1(data, clues)

def part_two(data, clues):
	return filter2(data, clues)

if __name__ == "__main__":
	clues = parse_clues(FILTER)
	data = parse_sues(DATA, clues)
	print(part_one(data, clues))
	print(part_two(data, clues))